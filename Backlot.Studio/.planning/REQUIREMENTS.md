...
- [x] **AUTH-01**: User can log in with username and password; credentials are base64-encoded as `username:password` and stored in server-side session as a Basic Auth header
- [x] **AUTH-02**: User can log out; server session is cleared and user is redirected to the login page
- [x] **AUTH-03**: User is automatically redirected to the login page when the Backlot API returns a 401 (expired or invalid credentials)
